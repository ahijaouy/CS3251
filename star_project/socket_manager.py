...
class SocketManager():
    def __init__(self, name, port, host=None, verbose=False):
    ...
    def start(self):
        listening_thread = Thread(target=self.sock.start_listening)
        listening_thread.start()
        self.log.debug("Socket Listening Thread started")
        sending_thread = Thread(target=self.sock.start_sending)
